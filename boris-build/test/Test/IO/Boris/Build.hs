...
prop_runBuild_happy =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_pre_fails_second =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_build_fails_first =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_build_fails_second =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_post_fails_first =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_post_fails_second =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_success_fails_first =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_success_fails_second =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_multi_fail_failure =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \t ->
...
prop_runBuild_multi =
  withTests 10 . property $ do
    b <- forAll Gen.genBuildName
    join . liftIO . withSystemTempDirectory "build"  $ \x -> do
      let
...
check :: FilePath -> (Either BuildError (), Bool, Bool, Bool, Bool, Bool) -> Specification -> IO (PropertyT IO ())
check x expected specification = do
  ...
  ok <- D.doesFileExist (t </> "success")
  ko <- D.doesFileExist (t </> "failure")

  pure $ (r, pre, command, post, ok, ko) === expected
...
o :: ConduitT ByteString o IO ()
o =
  CB.sinkHandle stdout

e :: ConduitT ByteString o IO ()
e =
