...
bare sout serr r target = do
  let local = LocalRepository $ Text.pack  target
  unlessM (liftIO $ doesDirectoryExist target) . X.hoistExitM $
    X.exec sout serr =<<
      X.xproc sout "git" ["clone", "--mirror", renderRepository r, Text.pack target]
  pure local
...
clone sout serr r target = do
  ...
  X.hoistExitM $
    X.exec sout serr =<<
      X.xproc sout "git" ["clone", renderLocalRepository r, Text.pack target]
  pure local
...
cloneref sout serr l r target = do
  ...
  X.hoistExitM $
    X.exec sout serr =<<
      X.xproc sout "git" ["clone", "--reference", renderLocalRepository l, renderRepository r, Text.pack target]
  pure local

checkout :: Out -> Out -> LocalRepository -> Ref -> EitherT ExitCode IO ()
checkout sout serr r ref =
  X.hoistExitM $
    X.exec sout serr . X.inDirectory (Text.unpack . renderLocalRepository $ r)  =<<
...
cat sout serr r ref f = do
  (out, _, c) <- liftIO $ X.capture sout serr . X.inDirectory (Text.unpack . renderLocalRepository $ r) =<<
...
refs sout serr r pattern = do
  let
    ...
    patternRecursive =
      if Text.isSuffixOf "/**" (renderPattern pattern) then
        [Text.replace "/**" "/*/*" (renderPattern pattern)]
      else
        []
  (out, _, c) <- liftIO $ X.capture sout serr . X.inDirectory (Text.unpack . renderLocalRepository $ r) =<<
    X.xproc sout "git" (["for-each-ref", "--format=%(refname)", renderPattern pattern] <> patternRecursive)
  X.hoistExit c
  pure $ fmap Ref . Text.lines . Text.decodeUtf8 $ out
...
commitAt sout serr r ref = do
  ...
  X.hoistExit c
  pure . Commit . Text.strip . Text.decodeUtf8 $ out
